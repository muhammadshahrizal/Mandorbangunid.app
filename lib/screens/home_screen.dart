...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> fetchServices() async {
    try {
      ...
      final response = await http
          .get(
            Uri.parse('http://192.168.1.23/mandorbangun.id/api/services.php'),
          )
      ...
      debugPrint('🚨 ERROR NARIK SERVICES: $e');
  ...
  Future<void> fetchProjects() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.1.23/mandorbangun.id/api/portfolio.php'),
      );
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
  ...
  IconData _mapIcon(String iconClass) {
    ...
    if (iconClass.contains('compass') || iconClass.contains('drafting')) {
